...
class TextPreprocessor:
    def __init__(self, db_handler: DatabaseHandler, sample_size: int = None):
    ...
    def _initialize_stanza_pipeline(self):
        try:
            self.nlp = stanza.Pipeline(lang=STANZA_LANG, processors=STANZA_PROCESSORS, use_gpu=True, verbose=False)
    ...
    def preprocess_dataset(self, dataset_name: str, split: str, sample_size: int = None,
        ...
        if not force_reprocess and self._check_processed_data_exists(dataset_name, split, suffix):
            return self._load_processed_data(dataset_name, split, suffix)

        sentences = self._load_sentences(dataset_name, split, suffix)
        if sentences.empty:
            return pd.DataFrame()

        parse_trees = self._process_sentences_with_stanza(sentences)
        parse_trees_df = pd.DataFrame(parse_trees)
        self._store_processed_data(parse_trees_df, dataset_name, split, suffix)
        return parse_trees_df

    def _check_processed_data_exists(self, dataset_name: str, split: str, suffix: str) -> bool:
        return self.db_handler.check_exists(dataset_name, split, f"parse_trees{suffix}")
    ...
    def prepare_sentence_pairs(self, split_data: pd.DataFrame, dataset_name: str, split: str) -> Tuple[
        ...
        self._store_sentence_pairs(pairs_df, sentences_df, dataset_name, split, len(split_data))
    ...
    def _store_sentence_pairs(self, pairs_df: pd.DataFrame, sentences_df: pd.DataFrame, dataset_name: str, split: str,
                              sample_size: int):
        suffix = f"_sample{sample_size}" if sample_size else ""
        self.db_handler.store_dataframe(pairs_df, dataset_name, split, f"pairs{suffix}")
        self.db_handler.store_dataframe(sentences_df, dataset_name, split, f"sentences{suffix}")
        logger.info(f"For dataset {dataset_name} and split {split}, created pairs_df and sentences_df")
...
#
#         suffix = f"_sample{len(split_data)}" if len(split_data) else ""
#
#         # Store in database
#         self.db_handler.store_dataframe(pairs_df, dataset_name, split, f"pairs{suffix}")
#         self.db_handler.store_dataframe(sentences_df, dataset_name, split, f"sentences{suffix}")
...
#         """
#         suffix = f"sample{total_samples}" if total_samples else ""
#
...
#
#         # Load full dataset
#         logger.info(f"Loading full dataset:train with syntactic parsing {dataset_name}")
#         full_dataset = self.db_handler.load_dataframe(dataset_name, "all", suffix)
#
#         if full_dataset.empty:
#             logger.warning(f"No data found for {dataset_name}")
#             return {}
...
#                 "val_indices": [val_idx]
#             })
#             self.db_handler.store_dataframe(fold_indices_df, dataset_name, f"cv_fold_{fold_index}",
#                                             f"indices_{suffix}")
#
#         # Store dataframes
#         self.db_handler.store_dataframe(stratified_sample, dataset_name, "stratified_sample", suffix)
#         self.db_handler.store_dataframe(train_split, dataset_name, "train_split", suffix)
#         self.db_handler.store_dataframe(test_split, dataset_name, "test_split", suffix)
#
#         return {
